...
export async function POST(request: Request) {
  try {
    ...
    if (!plan || !cardNumber || !name || !phone || !expiryDate || !cvv || !email) {
    ...
    const existEmail = await db.select().from(paymentTable).where(eq(paymentTable.email, email));
...
export async function DELETE(request: Request) {
  try {
    ...
    const result = await db.delete(paymentTable).where(eq(paymentTable.id, parseInt(id, 10))).returning();
    ...
    return new Response(
      JSON.stringify({ error: "Failed to remove order" }),
      { status: 500, headers: { "Content-Type": "application/json" } }
    );
  }
}
